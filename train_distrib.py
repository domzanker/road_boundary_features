...
# imports for distibuted training
import ignite.distributed as idist


def train(local_rank, opt, configs, **kwargs):
    ...
    if rank == 0:
        ...
        def log_tensorboard_images(engine):
            ...
            t_1 = make_grid(
                ground_trouth[:, 0:1, :, :], normalize=True, scale_each=True
            )
            """
...
def main(opt):
    ...
    with idist.Parallel(
        ...
        nproc_per_node=len(opt.gpu),
    ) as parallel:
        parallel.run(train, opt, configs)
